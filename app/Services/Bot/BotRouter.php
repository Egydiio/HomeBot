...
class BotRouter
{
    ...
    public function handle(Member $member, string $message, ?array $media): void
    {
        ...
        match ($currentState) {
            ...
            ConversationState::STATE_WAITING_CLASSIFICATION => $this->handleWaitingClassification($member, $message),

            ConversationState::STATE_WAITING_CONFIRMATION => $this->handleConfirmation($member, $message),

            ConversationState::STATE_WAITING_IMAGE_TYPE => $this->handleImageType($member, $message),

            ConversationState::STATE_WAITING_ITEM_CLASSIFICATION => $this->handleItemClassification($member, $message),

            ConversationState::STATE_WAITING_PAYMENT_SELECTION => $this->handlePaymentSelection($member, $message),
    ...
    private function handleConfirmation(Member $member, string $message): void
    {
        ...
        $transaction = Transaction::with('items')->find($data['transaction_id']);
    ...
    private function confirmMonthlyClosePayment(Member $member, MonthlyClose $close): void
    {
        try {
    ...
    private function handleBillConfirmation(Member $member, Transaction $transaction, string $text): void
    {
        if (in_array($text, ['sim', 's', 'yes', '✅'])) {
    ...
    private function zApi(): WhatsAppClientInterface
    {
        return app(WhatsAppClientInterface::class);
    }
